...
// Geocode using MapTiler — returns [{name, address, lat, lng}]
...
export default function LocationSearch({
  ...
  const [activeField, setActiveField] = useState("pickup"); // 'pickup' | 'dropoff' | null
  ...
  // Debounce
  useEffect(() => {
    const t = setTimeout(() => searchPickup(pickupQuery), 350);
    return () => clearTimeout(t);
  }, [pickupQuery]);

  useEffect(() => {
    const t = setTimeout(() => searchDropoff(dropoffQuery), 350);
    return () => clearTimeout(t);
  }, [dropoffQuery]);
  ...
  const selectPickup = (item) => {
    ...
    saveRecent(item);
    setRecent(getRecent());
    onPickup({ lat: item.lat, lng: item.lng });
  ...
  const selectDropoff = (item) => {
    ...
    saveRecent(item);
    setRecent(getRecent());
    onDropoff({ lat: item.lat, lng: item.lng });
  };
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        try {
          ...
          const item = { id: "my-location", name, address, lat, lng };
          setPickupSelected(item);
          setPickupQuery(name);
          setActiveField("dropoff");
          onPickup({ lat, lng });
  ...
  if (stage === "searching") {
    return (
      <motion.div
        ...
        animate={{ opacity: 1 }}
      >
        <div className="bg-zinc-50 dark:bg-zinc-800 rounded-3xl p-7 text-center">
        ...
        <button
          onClick={cancelRide}
          className="w-full py-3.5 rounded-2xl text-sm font-semibold text-red-500 bg-red-50 dark:bg-red-500/10 hover:bg-red-100 dark:hover:bg-red-500/20 transition-colors"
        >
          Cancel Request
        </button>
      </motion.div>
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        <div
          className={`flex items-center gap-3 px-4 py-3.5 rounded-2xl border-2 transition-all duration-200 bg-white dark:bg-zinc-800 ${activeField === "pickup" ? "border-brand shadow-brand/10 shadow-lg" : "border-zinc-100 dark:border-zinc-700"}`}
        >
          {/* Icon */}
          <div className="relative flex-shrink-0">
            <div
              className={`w-8 h-8 rounded-xl flex items-center justify-center transition-colors ${pickupSelected ? "bg-brand" : "bg-zinc-100 dark:bg-zinc-700"}`}
            >
              <MapPin
                size={14}
                className={pickupSelected ? "text-white" : "text-zinc-400"}
              />
            </div>
            {pickupSelected && (
              <motion.div
          ...
          <div className="flex items-center gap-1 flex-shrink-0">
            ...
            {pickupSelected && !loadingPickup && (
              <button
                onClick={clearPickup}
                className="w-6 h-6 rounded-full bg-zinc-100 dark:bg-zinc-700 flex items-center justify-center hover:bg-zinc-200 dark:hover:bg-zinc-600 transition-colors"
              >
            ...
            {!pickupSelected && !loadingPickup && (
              <button
                ...
                disabled={locating}
                className="w-7 h-7 rounded-xl bg-brand/10 flex items-center justify-center hover:bg-brand/20 transition-colors"
              >
                <Locate
                  size={13}
      ...
      <div className="flex items-center gap-3 px-6">
        <div className="flex flex-col gap-0.5">
          {[...Array(3)].map((_, i) => (
            <div
              key={i}
              className="w-0.5 h-1 bg-zinc-200 dark:bg-zinc-700 rounded-full"
            />
          ))}
        </div>
      </div>
      ...
      <div className="relative">
        <div
          className={`flex items-center gap-3 px-4 py-3.5 rounded-2xl border-2 transition-all duration-200 bg-white dark:bg-zinc-800 ${activeField === "dropoff" ? "border-red-400 shadow-red-400/10 shadow-lg" : "border-zinc-100 dark:border-zinc-700"}`}
        >
          <div className="relative flex-shrink-0">
            <div
              className={`w-8 h-8 rounded-xl flex items-center justify-center transition-colors ${dropoffSelected ? "bg-red-500" : "bg-zinc-100 dark:bg-zinc-700"}`}
            >
              <Navigation
                size={14}
                className={dropoffSelected ? "text-white" : "text-zinc-400"}
              />
            </div>
            {dropoffSelected && (
              <motion.div
      ...
      <AnimatePresence>
        {canRequest && (
          ...
          >
            {/* Route summary */}
            <div className="bg-zinc-50 dark:bg-zinc-800/60 rounded-2xl p-3.5 flex items-center gap-3">
              ...
              <div className="text-right flex-shrink-0">
                ...
                <p className="text-brand font-black text-base font-display">
                  ~
                </p>
              </div>
            </div>
            ...
            <button
              ...
              className="w-full py-2.5 rounded-xl text-xs font-semibold text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300 transition-colors"
            >
              Clear & start over
            </button>
          </motion.div>
...
function ResultRow({ item, onSelect, isRecent }) {
  return (
    ...
    >
      <div
        className={`w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 ${isRecent ? "bg-zinc-100 dark:bg-zinc-800" : "bg-zinc-100 dark:bg-zinc-800"}`}
      >
        {isRecent ? (
